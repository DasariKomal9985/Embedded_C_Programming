...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART1) {
		...
		HAL_UART_Receive_IT(&huart1, &rx_data, 1);               // Restart
	}
...
void SystemClock_Config(void) {
	...
	/** Initializes the RCC Oscillators (HSE -> PLL -> SYSCLK 84 MHz) */
	...
	RCC_ClkInitStruct.SYSCLKSource = RCC_SYSCLKSOURCE_PLLCLK;  // PLL as source
