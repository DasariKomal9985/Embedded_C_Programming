...
int main(void)
{
  ...
  while (1)
  {
	...
	readValueY = HAL_ADC_GetValue(&hadc4);
	HAL_Delay(100);
	buttonState = HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_5);
	if (buttonState == GPIO_PIN_SET)
	{
	    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_RESET);   // Set the pin high
	}
	else
	{
	    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET); // Set the pin low
	}
	HAL_Delay(100);
  }
  /* USER CODE END 3 */
